 ...
 * and specified color format. The pixels can then be "applied" or uploaded to
 * an OpenGL texture.
 * 
...
public class PixelData {
	...
	public static Texture createTexture(int width, int height,
			ImageData.Format format, int filter) throws SlickException {
		EmptyImageData data = new EmptyImageData(width, height);
		ByteBuffer dataBuffer = data.getImageBufferData();
		String ref = "pixelhandler:" + width + "x" + height + ":"
				+ format.toString();
		try {
			return InternalTextureLoader.get().createTexture(data, dataBuffer,
					ref, GL11.GL_TEXTURE_2D, filter, filter, false, format);
		} catch(IOException e) {
	 ...
	 * 
	 * @param width
	 *            the width in pixels of our data
	 * @param height
	 *            the height in pixels of our data
	 */
	 ...
	 * texture data, it will eventually be stored in OpenGL video memory using
	 * RGBA. For better performance and memory management, create textures with
	 * the same internal format as the format given to PixelData. The static
	 * 'createTexture' utility method is intended for this purpose.
	 * 
	 * Also note that most drivers will expand formats to RGBA internally, so
	 ...
	 *            the height in pixels of our data
	 * @param format
	 *            the desired format to use during uploading (RGB, RGBA, GRAY,
	 *            GRAYALPHA, ALPHA)
	 */
	...
	/**
	 * Uploads the pixel data to the given texture at the top-left origin (0,
	 * 0). This only needs to be called once, after the pixel data has changed.
	 * 
	 * @param texture
	 ...
	 *            the texture to modify
	 * @param x
	 *            the x position to place the pixel data on the texture
	 * @param y
	 *            the y position to place the pixel data on the texture
	 */
	public void apply(Texture texture, int x, int y) {
		if(x + width > texture.getTextureWidth()
				|| y + height > texture.getTextureHeight()) {
			throw new IndexOutOfBoundsException(
					"pixel data won't fit in given texture");
		}
		position(length());
		pixels.flip();
		...
		texture.bind();
		GL.glTexSubImage2D(SGL.GL_TEXTURE_2D, 0, x, y, width, height, glFmt,
				SGL.GL_UNSIGNED_BYTE, pixels);
	}
	...
	/**
	 * Clears the pixel array to the specified single-component color, i.e. to
	 * clear to white you would use:
	 * 
	 * <pre>
	 * clear(0xFF)
	 * </pre>
	 * 
	 * @param value
	 *            the byte value to fill the array with
	 * @return this object, for chaining
	 */
	...
	/**
	 * Copies this pixel data. Modifying the new data will not affect the
	 * original.
	 * 
	...
	Color getColor(byte[] array, int bpp, int offset, Color dst) {
		...
		if(bpp == 1) {
			return hasAlpha ? ret(dst, 0xFF, 0xFF, 0xFF, c1) : ret(dst, c1, c1,
					c1, 0xFF);
		}
	 ...
	 * 
	 * @param the
	 *            height of the pixel data region
	 */
	...
	/**
	 * Relative <i>get</i> method which handles color components based on image
	 * formats. Does not offset the buffer.
	 * 
	 * Color is converted like so based on the Format's color components: - If
	 * getColorComponents returns 1 and hasAlpha is true: Color(0xFF, 0xFF,
	 * 0xFF, A) - If getColorComponents returns 1 and hasAlpha is false:
	 * Color(L, L, L, 0xFF) - If getColorComponents returns 2: Color(L, L, L, A)
	 * - If getColorComponents returns 3: Color(R, G, B, 0xFF) - If
	 * getColorComponents returns 4: Color(R, G, B, A)
	 * 
	 * See setPixel for details.
	 * 
	 * @param dst
	 *            the color instance to place the values in
	...
	/**
	 * Loads the given texture into this PixelData. This is a somewhat expensive
	 * operation which may require copying all pixels from GPU memory to CPU
	 * memory (and incur a pipeline stall).
	 * 
	 * 
	 * 
	 * @param texture
	 *            the texture to grab pixels from
	 * @return the texture data array, for re-use
	 */
	public byte[]
			load(Texture texture, byte[] textureData, int srcx, int srcy,
		...
		if(dstx < 0f || dsty < 0f) {
			throw new UnsupportedOperationException(
					"negatives for dstx/dsty are not yet supported");
		}
		if(srcx < 0f || srcy < 0f) {
	...
	public byte[] load(Texture texture, ColorFilter filter) {
		return load(texture, 0, 0, texture.getImageWidth(),
				texture.getImageHeight(), filter);
	}

	public byte[] load(Texture texture, int srcx, int srcy, int srcWidth,
			int srcHeight) {
		return load(texture, srcx, srcy, srcWidth, srcHeight, 0, 0);
	}

	public byte[] load(Texture texture, int srcx, int srcy, int srcWidth,
			int srcHeight, ColorFilter filter) {
	...
	public byte[] load(Texture texture, int srcx, int srcy, int srcWidth,
			int srcHeight, int dstx, int dsty, ColorFilter filter) {
		return load(texture, texture.getTextureData(), srcx, srcy, srcWidth,
				srcHeight, dstx, dsty, filter);
	}

	/**
	 * Sets the position of this buffer, used alongside relative getPixel and
	 * putPixel methods.
	 * 
	 * @param pos
	 *            the position; the index for the buffer
	 */
	...
	/**
	 * Sets the position of the buffer to the offset given by the (x, y)
	 * coordinates, in pixels.
	 * 
	 * @param x
	 *            the x pixel to offset to
	 * @param y
	 *            the y pixel to offset to
	 */
	public void position(int x, int y) {
		if((x < 0) || (x >= width) || (y < 0) || (y >= height)) {
			throw new IndexOutOfBoundsException("Specified location: " + x
					+ "," + y + " outside of region");
		}
		int bpp = format.getColorComponents();
	...
	/**
	 * A convenience method to <i>put</i> a given pixel's color using Slick's
	 * color class. See the other putPixel method for details.
	 * 
	 * @param rgba
	 *            the RGBA components of the pixel
	 */
	public PixelData putPixel(Color rgba) {
		return putPixel(rgba.getRed(), rgba.getGreen(), rgba.getBlue(),
				rgba.getAlpha());
	}

	/**
	 * Relative <i>put</i> method which handles color components based on image
	 * formats. Does not offset the buffer.
	 * 
	 * See setPixel for details.
	 * 
	...
	/**
	 * Sets the pixel data to the given array, which should be less than the
	 * size of length().
	 * 
	 * @param pixelData
	 *            the new pixel data
	...
	/**
	 * A convenience method to set a given pixel's color using Slick's color
	 * class. See the other setPixel method for details.
	 * 
	 * @param x
	 *            the x position of the pixel
	 * @param y
	...
	public void setPixel(int x, int y, Color rgba) {
		setPixel(x, y, rgba.getRed(), rgba.getGreen(), rgba.getBlue(),
				rgba.getAlpha());
	}
	 ...
	 * 
	 * @param x
	 *            the x position to place the pixel
	 * @param y
	 *            the y position to place the pixel
	 * @param r
	 *            the red component
	 * @param g
	 *            the green component
	 ...
	 * 
	 * The Format determines what the resulting color will be: - If
	 * getColorComponents returns 1 and hasAlpha is true, 'a' will be used - If
	 * getColorComponents returns 1 and hasAlpha is false, 'rgb' will be
	 * averaged for luminance - If getColorComponents returns 2, 'rgb' will be
	 * averaged for luminance and and 'a' will be used for alpha - If
	 * getColorComponents returns 3 'r', 'g', 'b' will be used - If
	 * getColorComponents returns 4 'r', 'g', 'b' and 'a' will be used
	 * 
	 * In this fashion the following formats will produce expected results when
	 * using the values from Slick's Color class: RGBA, RGB, GRAYALPHA, GRAY,
	 * ALPHA.
	 * 
	 * @param x
	 *            the x position to place the pixel
	 ...
	 *            the y position to place the pixel
	 * @param r
	 *            the red, luminance or alpha component
	 * @param g
	 *            the green component
	 * @param b
	 *            the blue component
